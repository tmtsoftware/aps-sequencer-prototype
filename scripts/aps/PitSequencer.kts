...
script {
    ...
    loadScripts(pitCommon)

    onGoOnline {
        println("PitSequencer: sequencer going ONLINE")
    ...
    onGoOffline {
        println("PitSequencer: sequencer going OFFLINE")
        publishEvent(buildProcedureEvent(Prefix.apply(prefix),
            type      = ProcedureEventType.INFO_MESSAGE,
    ...
    onAbortSequence {
        println("PitSequencer: sequence ABORTED")
        publishEvent(buildProcedureEvent(Prefix.apply(prefix),
    ...
    onStop {
        println("PitSequencer: sequencer STOPPED")
        publishEvent(buildProcedureEvent(Prefix.apply(prefix),
            type      = ProcedureEventType.WARN_MESSAGE,
    ...
    onShutdown {
        println("PitSequencer: sequencer SHUTDOWN - cleaning up")
        publishEvent(buildProcedureEvent(Prefix.apply(prefix),
            type      = ProcedureEventType.INFO_MESSAGE,
    ...
    onGlobalError { error ->
        println("PitSequencer: unhandled error - ${error.reason}")
        publishEvent(buildProcedureEvent(Prefix.apply(prefix),
            type      = ProcedureEventType.WARN_MESSAGE,
